...
function [MAP_grain_sizes_in_microns, MAP_abundances_in_percent, MAP_endmembers_indices, R2, rmse] = ...
    main_function(results_dir, run_number, min_endmembers, max_endmembers, data, mixed_spectrum)
...
D = load_data(data, mixed_spectrum);
...
if R2 >= 0
    saveas(gcf, strcat(fit_dir, num2str(R2, '%.6f'), '_', num2str(run_number), '_BestFit_and_Residual.jpg'))
else
    saveas(gcf, strcat(fit_dir, '_', num2str(run_number), '_BestFit_and_Residual.jpg'))
end

% Output the data, model, component spectra, and weighted component spectra in ASCII format.
if R2 >= 0
    fileID = fopen(strcat(spec_dir, num2str(R2, '%.6f'), '_', num2str(run_number), '_Spectra.txt'), 'wt');
else
    fileID = fopen(strcat(spec_dir, '_', num2str(run_number), '_Spectra.txt'), 'wt');
end
[num_rows, ~] = size(lt);
...
% if R2 >= 0
%     saveas(gcf, strcat(cov_dir, num2str(R2, '%.6f'), '_', num2str(run_number), '_Distributions_and_Correlations.jpg'))
% else
%     saveas(gcf, strcat(cov_dir, '_', num2str(run_number), '_Distributions_and_Correlations.jpg'))
% end

% Save all of the relevant variables to reconstruct the results, along with confidence intervals.
if R2 >= 0
    save(strcat(var_dir, num2str(R2, '%.6f'), '_', num2str(run_number), '_Results.mat'), 'MODS', 'bestmod', 'D')
else
    save(strcat(var_dir, '_', num2str(run_number), '_Results.mat'), 'MODS', 'bestmod', 'D')
end
